...
class KMeansAnalysis:
    ...
    def run_kmeans_analysis(data_file, mode='lat_lon'):
        try:
            ...
            hist_clusters_file = f'tula_kmeans_{mode}_cluster_histogram.png'
            if include_methane:
                clusters = sorted(df[cluster_column].unique())
                data = [df[df[cluster_column] == cl]['methane3'] for cl in clusters]
                plt.figure(figsize=(10, 6))
                bins = np.histogram_bin_edges(df['methane3'], bins=30)
                plt.hist(
                    data,
                    bins=bins,
                    stacked=True,
                ...
                plt.grid(axis='y', linestyle='--', alpha=0.3)
                plt.tight_layout()
                plt.savefig(hist_clusters_file, dpi=300)
                plt.close()
                generated_files.append(hist_clusters_file)
            else:
                plt.figure(figsize=(8, 5))
                plt.bar(conteos.index.astype(str), conteos.values, color='tab:blue')
                plt.title(
                    f'Conteo de puntos por cluster (K={best_k}) - {mode}'
                )
                plt.xlabel('Cluster')
                plt.ylabel('Cantidad de puntos')
                plt.grid(axis='y', linestyle='--', alpha=0.3)
